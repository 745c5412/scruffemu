...
public class SocketManager
{
  ...
  public static void send(GameClient client, String packet)
  {
    if(client!=null&&client.getSession()!=null&&!client.getSession().isClosing()&&client.getSession().isConnected())
      client.send(packet);
    if(packet.length()<5)
      client.timeLastAct=System.currentTimeMillis();
  ...
  public static void GAME_SEND_cMK_PACKET_TO_ALL(Player perso, String suffix, int guid, String name, String msg)
  {
    ...
    if(perso.getLevel()<6)
    {
      SocketManager.GAME_SEND_MESSAGE(perso,"This channel can only be used from level <b>6</b> and up.");
      GAME_SEND_BN(perso);
      return;
    }
  ...
  public static void GAME_SEND_HDVITEM_SELLING(Player perso)
  {
    ...
    if(perso.getWorldMarket())
      entries=perso.getAccount().getHdvEntries(Config.getInstance().worldMarket); //R�cup�re un tableau de tout les items que le personnage � en vente dans l'HDV o� il est
    else
      entries=perso.getAccount().getHdvEntries(Math.abs(((Integer)perso.getExchangeAction().getValue()))); //R�cup�re un tableau de tout les items que le personnage � en vente dans l'HDV o� il est
