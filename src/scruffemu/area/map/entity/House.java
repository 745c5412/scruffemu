package scruffemu.area.map.entity;
...
public class House
{
  ...
  {
    if((!this.canDo(Constant.H_OCANTOPEN)&&(packet.compareTo(this.getKey())==0))||isHome)//Si c'est chez lui ou que le mot de passe est bon
    {
  ...
  public static void buy(Player player)//Acheter une maison
  {
    ...
    if(house.getOwnerId()>0)
    {
      ...
      if(seller.getCurrentPlayer()!=null)//FIXME: change the packet (Im)
        SocketManager.GAME_SEND_MESSAGE(seller.getCurrentPlayer(),"A house you owned has been sold for "+house.getSale()+" kamas.");
