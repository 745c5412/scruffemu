...
public class GameCase
{
  ...
  public void startAction(final Player player, GameAction GA)
  {
    ...
    if(actionID==-1)
    {
      SocketManager.GAME_SEND_MESSAGE(player,"Error: actionID is not defined.");
      return;
    }
    if(player.getDoAction())
    {
      SocketManager.GAME_SEND_MESSAGE(player,"You are already doing something. Please contact a staff member if the problem persists.");
      return;
    }
    ...
    switch(actionID)
    {
      case 62://Fontaine de jouvence
        if(player.getLevel()>5)
          return;
        SocketManager.GAME_SEND_MESSAGE(player,"The magic does its work and your health has been restored.");
        player.fullPDV();
        break;
      ...
      case 157: //Zaapis
        ...
        if(player.getCurMap().getSubArea().getArea().getId()==7&&(player.get_align()==1||player.get_align()==0||player.get_align()==3))//Ange, Neutre ou S�rianne
        {
          ...
          if(player.get_align()==1)
            price=10;
        } else if(player.getCurMap().getSubArea().getArea().getId()==11&&(player.get_align()==2||player.get_align()==0||player.get_align()==3))//D�mons, Neutre ou S�rianne
        {
