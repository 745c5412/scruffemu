...
import scruffemu.area.map.entity.InteractiveDoor;
import scruffemu.area.map.entity.InteractiveObject;
import scruffemu.area.map.entity.MountPark;
import scruffemu.area.map.entity.Trunk;
import scruffemu.area.map.entity.Tutorial;
import scruffemu.client.Account;
import scruffemu.client.Player;
import scruffemu.client.other.Party;
import scruffemu.command.CommandAdmin;
import scruffemu.command.CommandPlayer;
import scruffemu.command.administration.AdminUser;
...
import scruffemu.entity.npc.NpcAnswer;
...
import scruffemu.market.HdvLine;
...
import scruffemu.utility.NameGenerator;
...
public class GameClient
{
  ...
  private void boost(String packet)
  {
    try
    {
      ...
      {
        this.player.sendMessage("You are incarnated, you can not use your characteristics points in this state.");
        return;
      }
  ...
  private void sendTicket(String packet)
  {
    try
    {
      ...
      else
      {
        ...
        if(Config.getInstance().encryptPackets)
        {
  ...
  private void tchat(String packet)
  {
    ...
    switch(packet.charAt(2))
    {
      ...
      case '#'://Canal Equipe
        ...
        if(this.player.getFight()!=null)
        {
          ...
          if(team==-1)
            return;
          if(Logging.USE_LOG)
            Logging.getInstance().write("TeamMessage",this.player.getName()+" > "+this.player.getFight()+" > "+msg);
          if(this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF)!=null)
            if(this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF).getTemplate().getId()==10844)
              msg=Formulas.translateMsg(msg);
          SocketManager.GAME_SEND_cMK_PACKET_TO_FIGHT(this.player.getFight(),team,"#",this.player.getId(),this.player.getName(),msg);
      ...
      case '$'://Canal groupe
        ...
        msg=packet.split("\\|",2)[1];
        if(Logging.USE_LOG)
          Logging.getInstance().write("PartyMessage",this.player.getName()+" > "+this.player.getParty()+" > "+msg);
        if(this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF)!=null)
          if(this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF).getTemplate().getId()==10844)
            msg=Formulas.translateMsg(msg);
      ...
      case '@'://Canal Admin
        ...
        msg=packet.split("\\|",2)[1];
        if(Logging.USE_LOG)
          Logging.getInstance().write("AdminMessage",this.player.getName()+" > "+msg);
      ...
      case '?'://Canal recrutement
        ...
        else
        {
          ...
          msg=packet.split("\\|",2)[1];
          if(Logging.USE_LOG)
            Logging.getInstance().write("RecruitmentMessage",this.player.getName()+" > "+msg);
          if(this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF)!=null)
            if(this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF).getTemplate().getId()==10844)
              msg=Formulas.translateMsg(msg);
          SocketManager.GAME_SEND_cMK_PACKET_TO_ALL(this.player,"?",this.player.getId(),this.player.getName(),msg);
      ...
      case '%'://Canal guilde
        ...
        msg=packet.split("\\|",2)[1];
        if(Logging.USE_LOG)
          Logging.getInstance().write("GuildMessage",this.player.getName()+" > "+this.player.get_guild().getName()+" > "+msg);
        if(this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF)!=null)
          if(this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF).getTemplate().getId()==10844)
            msg=Formulas.translateMsg(msg);
      ...
      case '!'://Alignement
        ...
        msg=packet.split("\\|",2)[1];
        if(Logging.USE_LOG)
          Logging.getInstance().write("AlignMessage",this.player.getName()+" > "+msg);
        if(this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF)!=null)
          if(this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF).getTemplate().getId()==10844)
            msg=Formulas.translateMsg(msg);
  ...
  private void buy(String packet)
  {
    ...
    if(player.boutique)
    {
      try
      {
        ...
        if(points<=0)// Si le joueur n'a pas assez de point
        {
          ...
          SocketManager.GAME_SEND_MESSAGE(player,"You do not have enough votepoints to buy "+(quantity>1 ? "these items" : "this item")+", you currently have "+account.getPoints()+" votepoints and you need "+diferencia+" more.","FF0000");
        ...
        SocketManager.GAME_SEND_MESSAGE(player,"You have bought "+(quantity>1 ? "the items" : "the item")+": <b>"+template.getName()+"</b> for <b>"+value+"</b> votepoints!","000000");
      }
      catch(Exception e)
      {
        e.printStackTrace();
    ...
    else
    {
      ...
      try
      {
        ...
        {
          this.player.sendMessage("Thank you for not buying a single pet, it would get lonely.");
          return;
        }
        if(npc==null)
          return;
        ...
        if(template.getPoints()>0&&(npcTemplate.getInformations()&0x4)==4)
        {
          ...
          if(points<value)
          {
            SocketManager.GAME_SEND_MESSAGE(this.player,"You do not have enough votepoints to buy this item, you currently have "+points+" votepoints and you need "+(value-points)+" more to buy this item.");
            SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
            return;
          }
          ...
          SocketManager.GAME_SEND_MESSAGE(this.player,"You have "+(points-value)+" votepoints left.");
        }
  ...
  private void bigStore(String packet)
  {
    ...
    switch(packet.charAt(2))
    {
      case 'B': //Confirmation d'achat
        ...
        if(hL==null)
        {
          SocketManager.GAME_SEND_MESSAGE(this.player,"An error occurred during the confirmation of the purchase. Please contact a staff member.");
          return;
        }
        ...
        if(hE==null)
        {
          ...
          SocketManager.GAME_SEND_MESSAGE(this.player,"[2 - Template '"+hL.getTemplateId()+"'] An error occurred during the confirmation of the purchase. Please contact a staff member.");
          return;
        }
      ...
      case ExchangeAction.IN_MOUNT:
        ...
        switch(packet.charAt(2))
        {
          case 'O':// Objet
            ...
            {
              SocketManager.GAME_SEND_MESSAGE(this.player,"Dragoturkey inventory error 1: the item does not exist.");
              return;
            }
      ...
      case ExchangeAction.AUCTION_HOUSE_SELLING:
        switch(packet.charAt(3))
        {
          ...
          case '+'://Mettre un objet en vente
            ...
            {
              SocketManager.GAME_SEND_MESSAGE(this.player,"You have already reached the maximum amount of items you can sell at once.");
              return;
            }
            ...
            catch(Exception e)
            {
              ...
              e.printStackTrace();
              SocketManager.GAME_SEND_MESSAGE(this.player,"An error occured while selling your item. Please reconnect to fix the issue.");
              return;
            }
  ...
  private void offlineExchange()
  {
    ...
    if(taxe<0)
    {
      SocketManager.GAME_SEND_MESSAGE(this.player,"Merchant mode error: the tax is negative.");
      return;
    }
  ...
  private void request(String packet)
  {
    ...
    { // Craft s�curis� : celui qui � le job ( this.player ) souhaite invit� player
      try
      {
        ...
        if(!ok)
        {
          this.player.sendMessage("You are too far away from the workshop.");
          return;
        }
  ...
  private void bookOfArtisant(String packet)
  {
    switch(packet.charAt(2))
    {
      case 'F':
        ...
        if(cant==0)
          SocketManager.GAME_SEND_MESSAGE(this.player,"There is currently no craftsman available from the profession you are looking for.");
        break;
    }
  }
  ...
  private void joinWife(String packet)
  {
    ...
    switch(packet.charAt(2))
    {
      case 'S'://Teleportation
        ...
        {
          SocketManager.GAME_SEND_MESSAGE(this.player,"A magical barrier prevents you from joining your spouse.");
          return;
        }
        if(this.player.getFight()!=null)
          return;
  ...
  private void gameCheckSign(String packet)
  {
    ...
    if(qp==null)
      ...
      SocketManager.GAME_SEND_MESSAGE(this.player,"You have already started this quest.");
  ...
  private void placeTaxCollector()
  {
    ...
    if(this.player.getCurMap().getPlaces().length()<5||SoulStone.isInArenaMap(this.player.getCurMap().getId())||this.player.getCurMap().noCollector)
  ...
  private void banToGuild(String name)
  {
    ...
    //si quitter
    {
      ...
      {
        SocketManager.GAME_SEND_MESSAGE(this.player,"You must assign another leader to leave the guild.");
        return;
      }
      ...
      else
      {
          ...
          if(overExo!="")
          {
            ...
            for(int i=0;i<exos.size();i++)
            {
              ...
              if(Integer.valueOf(exoSplit.get(0))==111) //AP
              {
                ...
                {
                  this.player.sendMessage("You can not equip this item as you already have an AP exomaged item equipped.");
                  return;
                }
              ...
              if(Integer.valueOf(exoSplit.get(0))==128) //MP
              {
                ...
                {
                  this.player.sendMessage("You can not equip this item as you already have an MP exomaged item equipped.");
                  return;
                }
              ...
              if(Integer.valueOf(exoSplit.get(0))==117) //Range
              {
                ...
                {
                  this.player.sendMessage("You can not equip this item as you already have an Range exomaged item equipped.");
                  return;
                }
        ...
        if(object.getTemplate().getId()==2157)
        {
          if(position==Constant.ITEM_POS_COIFFE)
          {
            ...
            SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(this.player.getCurMap(),this.player.getId());
            SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(this.player.getCurMap(),this.player);
            SocketManager.GAME_SEND_MESSAGE(this.player,"You have been turned into a mercenary.");
          }
          else if(position==Constant.ITEM_POS_NO_EQUIPED)
          {
            ...
            SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(this.player.getCurMap(),this.player.getId());
            SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(this.player.getCurMap(),this.player);
            SocketManager.GAME_SEND_MESSAGE(this.player,"You are no longer a mercenary.");
          }
        }
  ...
  private void inviteParty(String packet)
  {
    ...
    if(target.getGroupe()!=null&&this.player.getGroupe()==null)
    {
      if(!target.getGroupe().isPlayer())
      {
        SocketManager.GAME_SEND_MESSAGE(this.player,"You can not invite staff members into your group.");
        return;
      }
    }
  ...
  private void buyMountPark(String packet)
  {
    ...
    if(Seller!=null)
    {
      ...
      if(Seller.isOnline())
      {
        SocketManager.GAME_SEND_MESSAGE(this.player,"You have sold your paddock for "+MP.getPrice()+" kamas.");
      }
    }
  ...
  private void sellMountPark(String packet)
  {
    ...
    {
      SocketManager.GAME_SEND_MESSAGE(this.player,"You can not sell a paddock that is not empty.");
      return;
    }
    if(MP1.getOwner()==-1)
    {
  ...
  public void changeName(String packet)
  {
    if(packet.equals("Af"))
    {
      this.player.setChangeName(false);
      return;
    }
    if(!this.player.hasItemTemplate(10860,1))
    {
      this.player.send("AlEr");
      this.player.sendMessage("You do not have a name change potion.");
      return;
    }
    String name="";
    name+=packet.substring(0,1).toUpperCase()+packet.substring(1).toLowerCase();
    boolean isValid=true;
    ...
    if(Database.getStatics().getPlayerData().exist(name)||!isValid)
    {
      ...
      this.player.sendMessage("The entered name is already in use or it is not allowed.");
      return;
    }
