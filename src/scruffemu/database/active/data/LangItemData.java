  ...
  public ResultSet load(int id)
  {
    ...
    try
    {
      ...
      while(RS.next())
      {
        try
        {
          Logging.getInstance().write("boutique","INSERT INTO site_shop_objects_templates VALUES('"+RS.getInt("id")+"', '"+RS.getInt("type")+"', '"+RS.getString("name").replace("{\"fr\":\"","").replace("\"}","").replace("'","\\'")+"', '"+RS.getString("description").replace("{\"fr\":\"","").replace("\"}","").replace("'","\\'")+"', "+"'"+RS.getInt("skin")+"', '"+RS.getInt("level")+"', '"+(RS.getString("effects")==null ? "" : RS.getString("effects").replace("null",""))+"');");
        }
        catch(Exception e)
        {
          e.printStackTrace();
