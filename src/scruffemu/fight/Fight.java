...
import scruffemu.fight.turn.Turn;
...
public class Fight
{
  ...
  public Fight(int id, GameMap map, Player perso, Collector perco)
  {
    ...
    if(getCollector()!=null)
      str="A"+getCollector().getN1()+","+getCollector().getN2()+"|.|"+World.world.getMap(getCollector().getMap()).getX()+"|"+World.world.getMap(getCollector().getMap()).getY();
  ...
  public void startFight()
  {
    ...
    if(this.collector!=null&&!this.collectorProtect)
    {
      ...
      for(Player player : protectors)
      {
        ...
        {
          SocketManager.GAME_SEND_MESSAGE(player,"You could not join the perceptor fight because you were unavailable.");
      ...
      switch(getType())
      {
        ...
        case Constant.FIGHT_TYPE_DOPEUL:
          ...
          else if(getState()==Constant.FIGHT_STATE_PLACE)
          {
            ...
            {// Il leave de son plein gr� donc (target = null)
              ...
              {// Soit il a lancer le combat => annulation du combat
                ...
                if(getType()==Constant.FIGHT_TYPE_PVT)
                {
                  ...
                  World.world.getMap(this.getCollector().getMap()).getPlayers().stream().filter(player -> player!=null).forEach(player -> SocketManager.GAME_SEND_MAP_PERCO_GMS_PACKETS(player.getGameClient(),player.getCurMap()));
                }
                setMap(null);
  ...
  public boolean canCastSpell1(Fighter caster, SortStats SS, GameCase cell, int targetCell)
  {
    ...
    else if(SS.hasLDV()&&!PathFinding.checkLoS(getMap(),casterCell,cell.getId(),caster))
    {
      if(perso!=null)
        SocketManager.GAME_SEND_Im_PACKET(perso,"1174");
      return false;
    ...
    try
    {
      ...
      else if(target.getTeam()==1)
      {
        ...
        for(Fighter fighter : team.values())
        {
          ...
          {// si il a �t� invoqu� par le joueur mort
    ...
    if((team0||team1||verifyStillInFight())&&!finish)
    {
      ...
      try
      {
        ...
        for(Fighter fighter : winTeam)
        {
          ...
          {
            World.world.getOnlinePlayers().stream().filter(player -> player!=null).filter(player -> player.get_align()==getPrism().getAlignement()).forEach(player -> SocketManager.SEND_CS_SURVIVRE_MESSAGE_PRISME(player,str));
      ...
      for(Fighter fighter : fighters)
      {
        ...
        if(!player.isFollowing)
        {
          if(Config.getInstance().teleports.containsKey((int)player.getCurMap().getId()))
          {
            ...
            player.teleport(player.getCurMap(),teleCell);
          }
        }
  ...
  public String getGE(int win)
  {
    ...
    try
    {
      ...
      for(Fighter fighter : loosers)
      {
        ...
        {
          for(int[] protector : scruffemu.job.JobConstant.JOB_PROTECTORS)
          {
      ...
      for(Fighter i : winners)
      {
        ...
        if(getType()==Constant.FIGHT_TYPE_PVT||getType()==Constant.FIGHT_TYPE_PVM||getType()==Constant.FIGHT_TYPE_CHALLENGE||getType()==Constant.FIGHT_TYPE_DOPEUL)
        {
          ...
          if(player!=null)
          {
            xpPlayer=Formulas.getXp(i,winners,totalXP,nbbonus,this.getMobGroup()!=null ? getMobGroup().getStarBonus(getMobGroup().getInternalStarBonus()) : 0,challXp,lvlMax,lvlMin,lvlLoosers,lvlWinners,World.world.getConquestBonus(player));
            XP.set(xpPlayer);
          ...
          else
          {
            ...
            temporary3.addAll(World.world.getEtherealWeapons(i.isInvocation() ? i.getInvocator().getLvl() : i.getLvl()).stream().map(objectTemplate -> new Drop(objectTemplate.getId(),0.001,0)).collect(Collectors.toList()));
            ...
            for(Drop drop : temporary3)
            {
              ...
              final double jet=Double.parseDouble(formatter.format(Math.random()*100).replace(',','.')),
                  chance=Formulas.dropChance((float)drop.getLocalPercent(),starFactor,challengeFactor,(float)World.world.getConquestBonus(player),(float)charPros,soloBonus);
              boolean ok=false;
              ...
              if(jet<chance||ok) //item gained
              {
                ...
                quantity=drop.getMinDrop()+(int)Math.rint(Math.random()*(drop.getMaxDrop()-drop.getMinDrop()));
      ...
      if(Collector.getCollectorByMapId(getMap().getId())!=null&&getType()==Constant.FIGHT_TYPE_PVM)
      {
        ...
        long winxp=Formulas.getXp(collector,winners,totalXP,nbbonus,(getMobGroup()!=null ? getMobGroup().getStarBonus(getMobGroup().getInternalStarBonus()) : 0),challXp,lvlMax,lvlMin,lvlLoosers,lvlWinners,0)/10;
